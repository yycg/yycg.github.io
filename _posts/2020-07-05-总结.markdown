...
4. 索引：B树索引和哈希索引
  ...
  - InnoDB：聚簇索引，树的叶节点data域保存了完整的数据记录。在根据主索引搜索时，直接找到key所在的节点即可取出数据；再根据辅助索引查找时，则需要先取出主键的值，再走一遍索引。
...
9. 数据库连接池：在连接池中，创建连接后，将其放置在池中，并再次使用它，因此不必建立新的连接。如果使用了所有连接，则会建立一个新连接并将其添加到池中。
